...
<div class="container-fluid">
    ...
    <div class="card">
        ...
        <div class="card-body" style="background: #fff;">
            <form role="form" method="post" action="scripts/host_insert.php" onsubmit="return upperMe1()" enctype="multipart/form-data">
                <div class="col-lg-4">
                    <label>
                    ...
                    <div class="form-group">
                        ...
                        <input style='font-size:15px;background-color:white;' class="form-control text-uppercase w3-border  w3-hover-border-blue" name="assoc_id" oninput='fetch(this.value)' name="assoc_id" placeholder="" required> </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label><b>Event Name</b></label>
                        <?php
                                $query = $link->query("SELECT DISTINCT(ename),slno FROM event  WHERE creator='$assoc_id' AND edate>='$today' ORDER BY slno DESC");
                                $rowCount = $query->num_rows;
                                ?>
                        <select style='font-size:15px;background-color:white;' class="form-control w3-border select2 w3-hover-border-blue" name="eventname" id="assoc_id" required>
                            <option value="" selected hidden>Upcoming Events</option>
                            <?php
                                  if(($rowCount > 0)){
                                  while($row = $query->fetch_assoc()){
...
$query2="SELECT * FROM `eventhost` ORDER BY event_id DESC";
...
while($arr2=  mysqli_fetch_row($result2)){
                        ...
                        <tr>
                            <td style="text-align:center">
                                <a class="btn btn-app" onclick="alertify.confirm('Please Confirm', 'You are trying to edit the Associate <?php echo $arr2[2];?>.', function(){ window.location='Edit_Host.php?slno=<?php echo $arr2[0]; ?>'}
                , function(){ });">
