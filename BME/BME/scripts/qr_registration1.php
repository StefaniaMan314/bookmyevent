...
if(intval($rows1[0])>=1){
 echo "<script>window.location='/BME/landing1_register.php?id=$event_id&assoc_id=$assoc_id'</script>";
}
else{
 ...
 $rows12=mysqli_fetch_row($check12);
    ...
    {
        $exist1="Already Full";
        header("location:/BME/index.php?status=Sorry, but the slot is full.");
    }
	else
		{
			
      $query2="INSERT INTO `testevent`(`AssociateID`, `AssociateName`, `email`, `Title`, `department`, `Organization`,  `Executive`, `timeslot`, `timestamp`,`event`, `timeslot1`, `booked`, `attend`, `attend_check`) VALUES ('$assoc_id','$assoc_name','$email','$role','$department','$Organization','$Executive','$timeslot','$timestamp','$eid','$slot1','1','1','$scanner_id')";
	  $result1 = $link->query($query2) or die("Error0 : ".mysqli_error($link));
...
$emailbox=$event_date1['emailbox'];
 ...
 if($food=='1'){
      echo "<script>window.location='/BME/foodselect_register.php?id=$event_id&assoc_id=$assoc_id'</script>";
	 } else {
		 echo "<script>window.location='/BME/landing_register.php?id=$event_id'</script>";
	 }
    }}
